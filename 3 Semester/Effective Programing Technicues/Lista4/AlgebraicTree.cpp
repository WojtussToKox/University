...
Result<AlgebraicTree*, Error> AlgebraicTree::buildTreeFromFormula(std::string formula)
{
	delete pRoot;
	pRoot = NULL;
	...
	pRoot = buildTreeRecursivly(formula, offset, vErrors, NULL);
	...
	if (offset < formula.size()) {
		std::string errorMess = "Za duzo zmiennych badz liczb na koncu formuly.";
		Error* pError = new Error(errorMess);
		vErrors.push_back(pError);
	}

	Result<AlgebraicTree*, Error> finalResult = Result<AlgebraicTree*, Error>::ok(this);
	
	if (!vErrors.empty()) {
		finalResult = Result<AlgebraicTree*, Error>::fail(vErrors);
...
Result<AlgebraicTree*, Error> AlgebraicTree::buildTree(std::string formula)
{
	AlgebraicTree* tempTree = new AlgebraicTree();

	// Wywo³ujemy prywatn¹ metodê na lokalnym obiekcie tempTree
	Result<AlgebraicTree*, Error> finalResult = tempTree->buildTreeFromFormula(formula);

	if (!finalResult.isSuccess()) {
		delete tempTree; // Jeœli b³¹d to trzeba usun¹æ wskaŸnik na drzewo temp
	}

	return finalResult;
}

Node* AlgebraicTree::buildTreeRecursivly(std::string& formula, int& offset, std::vector<Error*>& vErrors, Node* pParentNode)
{
	if (offset >= formula.size()) {
		std::string errorMess = "Brak zmiennej lub liczby na offsecie: " + std::to_string(offset);
		Error* pError = new Error(errorMess);
		vErrors.push_back(pError);
	...
	if (modified) {
		std::string errorMess = "Zmienna na offsecie: " + std::to_string(offset) + " zostala zmodyfikowana";
		Error* pError = new Error(errorMess);
		vErrors.push_back(pError);
	}
	...
	for (int i = 0; i < childrenCount; i++) {
		Node* pChild = buildTreeRecursivly(formula, offset, vErrors, pNewNode);
...
Node* AlgebraicTree::copyNodeRecursivly(Node* pOtherNode, Node* pParent, AlgebraicTree* pNewTree)
{
	...
	Node* pNewNode = new Node(*pOtherNode);

	pNewNode->setParent(pParent);

	updateTreePointerRecursivly(pNewNode, pNewTree);


	return pNewNode;
}

void AlgebraicTree::updateTreePointerRecursivly(Node* pNode, AlgebraicTree* pNewTree)
{
	if (pNode == NULL) return;

	pNode->setTree(pNewTree);

	for (int i = 0; i < pNode->getChildCount(); i++) {
		updateTreePointerRecursivly(pNode->getChild(i), pNewTree);
	}
}
...
//int AlgebraicTree::countLeafs()
//{
//	int result = 0;
//
//	result += countLeafsRecursivly(pRoot);
//
//	return result;
//}
...
int AlgebraicTree::countLeafs()
{
	return countLeafsRecursivly(pRoot);
}


int AlgebraicTree::countLeafsRecursivly(Node* pNode)
{
	if (pNode->getChildCount() == 0) {
		return 1;
	}
	int result = 0;
	for (int i = 0; i < pNode->getChildCount(); i++) {
		result += countLeafsRecursivly(pNode->getChild(i));
	}
	return result;
}